...
- **生成テスト**: `verification/checks/verify-generated-compile.sh` を **1.5** に強化。
...
用い、`verification/checks/verify-eclipse-compile.sh` で検証した。
...
| `verification/checks/verify-generated-compile.sh`(1.5) | **OK**(s2dao 8 + tiger 5、major 49) |
| `verification/checks/verify-eclipse-compile.sh`(ECJ 1.5、UTF-8/MS932) | **OK**(エラーゼロ) |
| `verification/run/run-old-env.sh`(H2 Oracle 互換、-source 1.5) | **PASS**(21 + 20) |
| `verification/run/run-new-env.sh`(PostgreSQL16) | **PASS**(21 + 20) |
| `verification/run/compare/compare.sh` | **PASS**(比較 75 / 一致 75 / 不一致 0) |
